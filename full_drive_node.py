        ...
        if self._processing:
            ...
            if self._state == 'RACE' and self._tf_control:
                ...
                if self._gyro_cnt >= 1:
                    ...
                    if abs(self._total_heading_change) >= 80 and self._front_dist < 1.5:
                        self._corner_cnt +=1
                        self.get_logger().info(f"Number of corners {self._corner_cnt} heading {self._total_heading_change}")
                        self._total_heading_change = 0

                    if self._parking_lot > 50 and self._corner_cnt == 12 and abs(self._total_heading_change) > 70 and self._parking_lot_detect and self._front_dist < 2.0:  #430
